...
let mul (p : t) (q : t) =
  let p2,p3,p4,p1 = Fvec4.to_tuple p
  and q2,q3,q4,q1 = Fvec4.to_tuple q
  in
  Fvec4.of_tuple (
    (p2 *. q1) +. (p1 *. q2) +. (p3 *. q4) -. (p4 *. q3),
    (p3 *. q1) +. (p1 *. q3) +. (p4 *. q2) -. (p2 *. q4),
    (p4 *. q1) +. (p1 *. q4) +. (p2 *. q3) -. (p3 *. q2),
...
let to_matrix (q : t) =
  let s = let n = norm q in if n > 0. then 2. /. n else 0. in
  let x,y,z,w = to_tuple q in
  let xs = x *. s and ys = y *. s and zs = z *. s in
  ...
  M.set m 0 0 (1.0 -. yy -. zz);
  M.set m 0 1 (xy +. wz);
  M.set m 0 2 (xz -. wy);
(*   M.set m 0 3 0.0; *)

  M.set m 1 0 (xy -. wz);
  M.set m 1 1 (1.0 -. xx -. zz);
  M.set m 1 2 (yz +. wx);
(*  M.set m 1 3 0.0; *)

  M.set m 2 0 (xz +. wy);
  M.set m 2 1 (yz -. wx);
  M.set m 2 2 (1.0 -. xx -. yy);
(*  M.set m 2 3 0.0; *)
...
let of_matrix (m : M.Mat4.t) =
  ...
  then
    ...
    let compute m f a b c =
      let s = sqrt ((M.get m a a) -. (M.get m b b) -. (M.get m c c)) +.
        (M.get m 3 3) in
    ...
    match M.get m 0 0, M.get m 1 1, M.get m 2 2 with
      | x,y,z when (y > x) && (z > x) ->
        fix (compute m (fun w x y z -> of_tuple (x,y,z,w)) 0 1 2)
      | x,y,z when (y > x) && (z < x) ->
        fix (compute m (fun w x y z -> of_tuple (y,z,x,w)) 1 2 0)
      | _                             ->
        fix (compute m (fun w x y z -> of_tuple (z,x,y,w)) 2 0 1)
