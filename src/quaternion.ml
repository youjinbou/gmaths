...
let test =
  ...
  x *. pi /. 180.
  in
  let check_float x r precision =
    assert ( abs_float (x -. r) <  precision ) 
  in
  let theta = deg_to_rad 30.
  and axis  = 1. /. (sqrt 41.), -.2. /. (sqrt 41.), 6. /. (sqrt 41.) in
  let p = Fvec4.of_tuple (3.,7.,-.2.,1.0) 
  and q = of_axis_angle axis theta in
  let rq = 
    let sn = sin (deg_to_rad 15.)
    and sq = sqrt 41. in
    Fvec4.of_tuple (
      sn /. sq,
      -.2. *. sn /. sq,
  ...
  in
  let r = rotate q p in
  let x',y',z',w' = Fvec4.to_tuple r in
  prerr_endline "quaternion test ------------------------";
  prerr_endline (Printf.sprintf "quat        : %s" (Fvec4.to_string q));
  prerr_endline (Printf.sprintf "expected    : %s" (Fvec4.to_string rq));
  prerr_endline (Printf.sprintf "norm        : %f" (norm rq));  
  prerr_endline (Printf.sprintf "test result : %s" (Fvec4.to_string r));
  prerr_endline (Printf.sprintf "expected result : < %f ; %f ; %f ; %f > " (-0.44438) (7.774228) (-1.16786) 1.0);
  prerr_endline "quaternion test ------------------------";
  let prec = 0.001 in
  check_float x' (-0.44438) prec;
  check_float y' (7.774228) prec;
  check_float z' (-1.16786) prec;
  check_float w' 1.0 prec
*)
